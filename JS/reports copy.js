...
async function renderPosTable(page = 1, pageSize) {
  ...
  try {
    ...
    const posTransactionData = await getPosTransactions(shopId);

    const posTransactions = posTransactionData.data.transactions;

    totalPages = posTransactionData.data.totalPages;
